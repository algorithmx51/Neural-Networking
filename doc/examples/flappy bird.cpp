...
void Bird::draw(int color)// draw the bird with color "color"
{
    ...
    setfillstyle(SOLID_FILL, color);
    floodfill(this->x, this->y-9, color);
...
void think(network **BRAIN) // function to decide the movement of each bird
{
    ...
    float INP[4] = {0, 0, 0, 0}; // a tmp list of floats
    ...
    for(int i = 0; i < POPULATION; i++) // loop through the population
    {
        if(b[i].dead == 0) // if that bird is not dead
        {
            ...
            INP[1] /= 1000;                                                                                             /// Input layer : 1. distance from the closest pipe , 2. distance on the y from the upper part of the closest pipe  ,  3. distance on the y from the lower part of the closest pipe and 4. the Y velocity of the bird
            INP[2] = b[i].y - (getmaxy() - p[index].length);                                            /// Hidden layers : 1
...
void handle_user_input(char ch) // function to handle user input
{
    ...
    case 's': // save the state of the neural network
        ...
        // open "BESTNW.bin" to write , or best-neural-network
        exportNet(brain[index], "BESTNW.bin"); // export it
        exit(0);
        break;
...
int main(void) /// MAIN
{
    ...
    int w1, w2, nmap[] = {4, 3, 2}; // w1, w2 : integers to store the ids of the windows that will be created, nmap : network map -> how many neurons on each layer
    ...
    w2 = initwindow(600, 460, "Neural Network of the Best Player", 1000, 0); // create a window to display the "brain" of the best player
    w1 = initwindow(1000, 600, "Flappy Bird AI", 0, 0); // create a window to display the game
    ...
    while(1)
    {
        ...
        dead = 0; // we start over, so our birds are alive
        ...
        while(!dead)
        {
            ...
            setfillstyle(SOLID_FILL, RGB(0, 255, 255));
            floodfill(0, 0, WHITE);

            for(int i = 0; i < pipes; i++) // draw the pipes
            {
                p[i].draw(RGB(0, 220, 0));
            }
            ...
            sprintf(buffer, "Generation %d", gen);
